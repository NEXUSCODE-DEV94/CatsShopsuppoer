...
class CancelModal(discord.ui.Modal, title='キャンセル理由の入力'):
    reason = discord.ui.TextInput(label='キャンセル理由', style=discord.TextStyle.paragraph, placeholder='理由を入力してください', required=True)
...
class AdminControlView(discord.ui.View):
    ...
    @discord.ui.button(label="受け取り完了", style=discord.ButtonStyle.green, custom_id="norm_admin_receive")
    ...
    @discord.ui.button(label="商品を配達", style=discord.ButtonStyle.blurple, custom_id="norm_admin_deliver")
    ...
    @discord.ui.button(label="キャンセル", style=discord.ButtonStyle.danger, custom_id="norm_admin_cancel")
...
class ConfirmView(discord.ui.View):
    def __init__(self, item_name, price, item_data):
        ...
        self.price = price
        self.item_data = item_data
    @discord.ui.button(label="購入を確定", style=discord.ButtonStyle.green, custom_id="norm_confirm_purchase")
...
class ItemSelectView(discord.ui.View):
    def __init__(self, items_dict):
        ...
        options = [discord.SelectOption(label=name, description=f"価格: {data['price']}円", emoji="<a:win11:1463441603057422419>") for name, data in items_dict.items()]
        self.select = discord.ui.Select(placeholder="商品を選択してください", options=options, custom_id="norm_item_select")
...
class VendingView(discord.ui.View):
    def __init__(self, items=None):
        super().__init__(timeout=None)
        self.items = items
    @discord.ui.button(label="購入", style=discord.ButtonStyle.green, custom_id="norm_purchase_btn")
    async def purchase_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(embed=discord.Embed(description="商品を選択してください。", color=discord.Color.green()), view=ItemSelectView(self.items), ephemeral=True)

async def setup(bot: commands.Bot):
    @bot.tree.command(name="vending-n2", description="はん自販機パネルを設置します")
    async def vending_cmd(interaction: discord.Interaction):
        if not os.path.exists("items.json"):
            await interaction.response.send_message("items.jsonが見てかりません。", ephemeral=True)
            return
        with open("items.json", "r", encoding="utf-8") as f: items = json.load(f)
        await interaction.response.send_message("設置完了！", ephemeral=True)
        embed = discord.Embed(title="通常自販機", description="商品を選択してください", color=discord.Color.green())
        for name, data in items.items(): embed.add_field(name=name, value=f"価格: {data.get('price', 0)}円", inline=False)
