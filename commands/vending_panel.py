...
class VendingSelect(ui.Select):
    def __init__(self):
        ...
        super().__init__(placeholder="商品を選択してください", options=options, min_values=1, max_values=1, custom_id="vending_select_menu")
    ...
    async def callback(self, interaction: Interaction):
        ...
        try:
            ...
            await interaction.response.send_message("DM送信に失敗しました。DM設定を解放してください。", ephemeral=True)

class VendingView(ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    @ui.button(label="🛒 購入", style=discord.ButtonStyle.green, custom_id="vending_buy_btn")
...
async def setup(bot):
    ...
    async def vending_panel(interaction: Interaction):
        embed = discord.Embed(title="無料自販機", description="下記ボタンを押して購入したい商品を選択してください", color=discord.Color.green())
